...
- 三个 Vorbis 包头
- codebook（Huffman + VQ lookup type 1/2，含 `sequence_p`）、floor 1、residue type 0/1/2
- 立体声耦合、IMDCT、含长短块切换的重叠相加
- 输出多声道 16-bit PCM WAV

尚未支持：floor 0。

验证覆盖：6 个真实文件（11025 / 44100 / 96000 Hz，单声道与立体声）相关系数 0.9987–0.9998；生成的素材补齐了真实文件走不到的分支——granule 裁剪 12 组、residue type 0/1 单声道与多声道与 `sequence_p` 共 14 组，与 libvorbis 的相关系数均为 1.000000。
