...
> 纯 MoonBit OGG/Vorbis 解码器开发中，用 stb_vorbis 做交叉验证揪出 7 处规范偏差、外加一处解码流程坑的经历。
...
```
单声道 正弦波, 44100 Hz, 原始 44100 帧, 解码 44608 帧
  声道 0:  相关系数 0.9999   RMS 误差 0.0047   OK

立体声 正弦波, 44100 Hz, 原始 44100 帧, 解码 44608 帧
  声道 0:  相关系数 0.9999   RMS 误差 0.0047   OK
  声道 1:  相关系数 1.0000   RMS 误差 0.0041   OK

立体声 噪声, 44100 Hz, 原始 44100 帧, 解码 44160 帧
...
残余误差有三个来源，都在预期内：Vorbis 本身是有损压缩（我对比的是解码结果而非原始波形）；float32 精度；以及首帧的 overlap 偏移——解码输出 44608 帧而原始 44100 帧，多出的是编解码器固有的前后沿填充，脚本按较短长度对齐后再比较。
